...
| 24 | 2026-08-31 | Default store location | `store:` defaults to the directory the config file was found in | A fixed `~/.ticket` constant independent of where the config is | `$TICKET_CONFIG=/opt/team/config.yml` otherwise moves the config but leaves state in `~/.ticket`. Deriving it means one env var relocates the whole root; the documented default is unchanged, since `~/.ticket/config.yml` still yields `~/.ticket` | firm |
| 25 | 2026-08-31 | Relative paths in config | `store:`, `worktrees.root:`, and `repos.<repo>.path:` anchor to the config directory, via the same `_anchor` helper as `prompt:`/`run:` | Resolving them against the current working directory | `store: state` would otherwise name a different directory per invocation. One rule for every path in the file is the only rule worth documenting | firm |
| 26 | 2026-08-31 | XDG base directories | Not supported; `$TICKET_CONFIG` and `$TICKET_STORE` are the knobs | Honour `$XDG_CONFIG_HOME` / `$XDG_STATE_HOME` | Single-user macOS tool. A third resolution path costs branches and tests for no user; #24 already gives one-variable relocation. Revisit if it ever ships to Linux | provisional |
| 27 | 2026-08-31 | Repo override validation | Eager — every `repos:` entry is checked (via `for_repo`) at `load_config` time, not lazily when a run first touches that repo | Lazy — validate a repo override only when a command scopes to it | A bad override for a repo nobody is currently touching would otherwise pass silently and only surface days later on the first `ticket next` against it. Failing at load time means every command on the machine refuses immediately, at the cost of validating repos a given run may never use | firm |
