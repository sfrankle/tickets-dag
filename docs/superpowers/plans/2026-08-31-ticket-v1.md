...
- Produces:
  ...
  - `ticket.config.Config` — frozen dataclass: `store: Path` (defaults to `root`, decision #24), `root: Path` (directory the config file lives in; all relative `store`/`worktrees.root`/`prompt`/`run` paths resolve against it, decision #25), `models: dict[str, str]`, `default_model: str`, `steps: tuple[Step, ...]`, `reviews: tuple[Review, ...]`, `repos: dict[str, dict]`, `worktrees: Worktrees`, `sync: bool`
  ...
  - `ticket.config._anchor(raw: str, root: Path) -> Path` — expand `~`, anchor relative paths to the config directory
