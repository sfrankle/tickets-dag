...
- **Severity and class are orthogonal and never derived from each other.** 🔴/🟡/🔵 answer *how important is this to fix*. `easy`/`hard` answer *who can fix it* — `easy` means the gh bot can do it from an `/edit` comment, `hard` means it needs a local Claude session. A 🔴 can be a one-line fix and a 🔵 can be a rewrite. No code anywhere maps one onto the other.
- **Decision #16 supersedes spec §8.1.** A finding's `class` is set by Haiku at ingestion, for every finding from every source. The severity rule described in spec §4 ("🔵/🟡 → easy, 🔴 → hard") is **not implemented at all**, not even as a fallback — see the constraint above. When the classifier cannot be reached, the finding is stored with `class: null` and `ticket findings` shows it as unclassified; the resolver leaves it for last and `fix` refuses to route it until `ticket class` sets one by hand.
...
src/ticket/classify.py      Haiku classification of finding class (decision #16)
src/ticket/fix.py           route a finding by class, enforce ordering, verify trailer
...
`gh.py` and `classify.py` are additions to the module table in spec §7. `gh.py` exists so the retry-three-times-with-backoff rule lives in one place instead of being repeated in `reviews.py`, `collect.py`, and `fix.py`. `classify.py` exists because decision #16 moved classification off the parse path — parsing and classifying now have different failure modes and different cost profiles, so they get different files.
...
2. **`fix` prefers `easy` findings, and offers unclassified ones last.** Spec §4 requires remote work before local work so a local commit never lands on a head the bot then moves past. The resolver returns the lowest-numbered open `easy` finding, then `hard`, then `class: null`. Severity plays no part in this ordering: it measures importance, not effort.
...
def test_collect_beats_an_open_finding(cfg):
    ticket = ticket_doc(done("evaluate", "spec", "draft-pr"), prs=["acme/api#115"])
    pr = pr_doc(dispatched=[{"review": "docs-tests", "head": "9c1f0ab", "transport": "bot"}])
    findings = findings_doc({"id": "f01", "status": "open", "class": "easy"})
...
def test_an_open_finding_beats_the_next_review(cfg):
    ...
    findings = findings_doc({"id": "f01", "status": "open", "class": "hard"})
    action = next_action(cfg, ticket, pr, findings)
...
def test_easy_findings_are_offered_before_hard_ones(cfg):
    ...
    findings = findings_doc(
        {"id": "f01", "status": "open", "class": "hard"},
        {"id": "f02", "status": "open", "class": "easy"},
    )
    assert next_action(cfg, ticket, pr, findings).target == "f02"


def test_unclassified_findings_are_offered_last(cfg):
    """class is never inferred from severity, so an unclassified finding waits."""
    ticket = ticket_doc(done("evaluate", "spec", "draft-pr"), prs=["acme/api#115"])
    pr = pr_doc(
        dispatched=[{"review": "docs-tests", "head": "9c1f0ab", "transport": "bot"}],
        collected=[{"source_id": "PRR_1", "review": "docs-tests", "findings": ["f01", "f02"]}],
    ...
    findings = findings_doc(
        {"id": "f01", "status": "open", "class": None, "severity": "blocking"},
        {"id": "f02", "status": "open", "class": "hard", "severity": "architecture"},
    )
    assert next_action(cfg, ticket, pr, findings).target == "f02"
...
def _open_finding(findings: dict | None) -> dict | None:
    ...
    # never lands on a head the bot then moves past. Unclassified findings go
    # last: class is never inferred from severity, so they need `ticket class`.
    order = {"easy": 0, "hard": 1}
    is_open.sort(key=lambda f: (order.get(f.get("class"), 2), f["id"]))
    return is_open[0]
    ...
    if has_pr:
        ...
        if finding:
            return Action(
                "fix",
                finding["id"],
                f"{finding['id']} is open ({finding.get('class', 'unclassified')})",
            )
...
def test_script_never_derives_class():
    """Severity is importance, not effort. Nothing here may set class."""
    assert all("class" not in f for f in parse_script(body("example-review.md")))
...
This module never sets `class`. Severity says how important a finding is;
class says who can fix it. See classify.py.
"""
...
### Task 9: Classifying findings (decision #16)

**Files:**
- Create: `src/ticket/classify.py`
- Create: `tests/test_classify.py`

**Interfaces:**
- Consumes: `Config`, `ticket.errors.TicketError`.
- Produces:
  - `ticket.classify.CLASSES` — `("easy", "hard")`
  - `ticket.classify.classify(cfg: Config, findings: list[dict]) -> list[dict]` — sets `class` on each finding in place and returns the list. One Haiku call for the whole batch. On any failure, sets `class` to `None` and does not raise.

**What `class` means, and what it does not.** `easy` means the gh Claude bot can fix it from an `/edit` comment: a contained, mechanical change. `hard` means it needs a local Claude session: it requires judgement, spans files, or changes behaviour. This is a question about **effort and containment**, not importance. Severity (🔴/🟡/🔵) is never an input to it — a 🔴 can be a one-line fix and a 🔵 can be a rewrite — so the prompt below does not receive the severity field at all. That omission is deliberate; do not add it.

**Failure is soft.** A finding that cannot be classified is stored with `class: null`, the resolver offers it last (Task 3), and `fix` refuses to route it (Task 11) until `ticket class` sets one by hand (Task 12). There is no severity fallback (decision #18).
...
from ticket.classify import classify
from ticket.config import load_config
...
def test_classify_sets_class_from_the_model(cfg, fake_bin):
    fake_bin.respond("claude", stdout=json.dumps(["easy", "hard"]))
    result = classify(cfg, findings())
    assert [f["class"] for f in result] == ["easy", "hard"]


def test_classify_uses_haiku(cfg, fake_bin):
    fake_bin.respond("claude", stdout=json.dumps(["easy", "hard"]))
    classify(cfg, findings())
    assert "claude-haiku-4-5-20251001" in fake_bin.calls_to("claude")[0]


def test_classify_makes_one_call_for_the_batch(cfg, fake_bin):
    fake_bin.respond("claude", stdout=json.dumps(["easy", "hard"]))
    classify(cfg, findings())
    assert len(fake_bin.calls_to("claude")) == 1


def test_severity_is_not_sent_to_the_classifier(cfg, fake_bin):
    """Severity is importance, not effort. It must not influence class."""
    fake_bin.respond("claude", stdout=json.dumps(["easy", "hard"]))
    classify(cfg, findings())
...
def test_a_model_failure_leaves_class_null(cfg, fake_bin):
    fake_bin.respond("claude", exit_code=1, stderr="unavailable")
    result = classify(cfg, findings())
    assert [f["class"] for f in result] == [None, None]


def test_junk_output_leaves_class_null(cfg, fake_bin):
    fake_bin.respond("claude", stdout="not json")
    assert [f["class"] for f in classify(cfg, findings())] == [None, None]


def test_a_wrong_length_answer_leaves_class_null(cfg, fake_bin):
    fake_bin.respond("claude", stdout=json.dumps(["easy"]))
    assert [f["class"] for f in classify(cfg, findings())] == [None, None]


def test_an_unrecognised_class_becomes_null(cfg, fake_bin):
    fake_bin.respond("claude", stdout=json.dumps(["easy", "medium"]))
    assert [f["class"] for f in classify(cfg, findings())] == ["easy", None]
```

- [ ] **Step 2: Run the tests to verify they fail**

Run: `uv run pytest tests/test_classify.py -v`
Expected: collection error, `No module named 'ticket.classify'`.

- [ ] **Step 3: Write `src/ticket/classify.py`**

```python
"""Set a finding's class with Haiku, at ingestion (decision #16).
...
Failure is soft: an unclassifiable finding gets class None, the resolver offers
it last, and `fix` refuses to route it until `ticket class` sets one by hand.
"""
...
def classify(cfg: Config, findings: list[dict]) -> list[dict]:
    if not findings:
        return findings
    ...
    for finding, answer in zip(findings, answers):
        finding["class"] = answer if answer in CLASSES else None
    return findings
```
...
```bash
git add src/ticket/classify.py tests/test_classify.py
git commit -m "feat: classify findings easy/hard with Haiku at ingestion" -m "Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>"
```
...
**Interfaces:**
- Consumes: `ticket.gh`, `ticket.parse`, `ticket.classify`, `Store`.
- Produces:
...
def test_collected_findings_are_classified(cfg, store, fake_bin):
    seeded_pr(store)
    body = (FIXTURES / "example-review.md").read_text()
    fake_bin.respond("gh api repos/acme/api/pulls/115/reviews", stdout=review_payload(body))
    fake_bin.respond("claude", stdout=json.dumps(["easy", "hard", "easy"]))
    collect(cfg, store, ticket_doc(), "acme/api#115")
    classes = [f["class"] for f in store.read_findings("acme/api#115")["findings"]]
    assert classes == ["easy", "hard", "easy"]
...
git commit -m "feat: collect reviews and comments, parse and classify findings" -m "Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>"
```
...
- Produces:
  ...
  - `ticket.fix.set_class(store, pr_ref, finding_id, value: str) -> None`
...
**Routing is a property of the finding, not of the call** (spec §4). `fix` never takes a class argument. Letting a caller send a `hard` finding to `/edit` fails quietly — it produces a minimal diff that misses the point — so a finding with `class: null` is refused outright with the command to set one.
...
def test_resolve_from_git_marks_findings_resolved(cfg, store, worktree):
    seed(store, {"summary": "a", "class": "easy"})
    subprocess.run(
...
def test_easy_finding_goes_to_the_bot(cfg, store, worktree, fake_bin):
    seed(store, {"summary": "README names a removed flag", "class": "easy", "file": "README.md"})
    fake_bin.respond("gh pr view", stdout=json.dumps({"headRefOid": "bbb"}))
...
def test_hard_finding_runs_a_local_session_and_commits(cfg, store, worktree, fake_bin):
    seed(store, {"summary": "retry loop unbounded", "class": "hard", "file": "src/api/retry.py"})
    fix_one(cfg, store, ticket_doc(worktree), "acme/api#115", "f01")
    assert fake_bin.calls_to("claude")
...
def test_a_hard_finding_uses_the_default_model(cfg, store, worktree, fake_bin):
    seed(store, {"summary": "retry loop unbounded", "class": "hard"})
    fix_one(cfg, store, ticket_doc(worktree), "acme/api#115", "f01")
    assert "claude-opus-5" in fake_bin.calls_to("claude")[0]


def test_an_unclassified_finding_is_refused(cfg, store, worktree, fake_bin):
    seed(store, {"summary": "unknown effort", "class": None})
    with pytest.raises(TicketError, match="ticket class"):
        fix_one(cfg, store, ticket_doc(worktree), "acme/api#115", "f01")
    assert fake_bin.calls == []
...
def test_set_class_overrides(cfg, store):
    seed(store, {"summary": "a", "class": None})
    set_class(store, "acme/api#115", "f01", "hard")
    assert store.read_findings("acme/api#115")["findings"][0]["class"] == "hard"


def test_set_class_rejects_an_unknown_value(cfg, store):
    seed(store, {"summary": "a", "class": None})
    with pytest.raises(TicketError, match="easy"):
        set_class(store, "acme/api#115", "f01", "medium")


def test_dry_run_posts_nothing_and_changes_nothing(cfg, store, worktree, fake_bin):
    seed(store, {"summary": "a", "class": "easy", "file": "README.md"})
...
```python
"""Route a finding by class, enforce ordering, verify the trailer landed.
...
and this module commits. A finding with no class is refused rather than guessed
...
def fix_one(
    ...
    *,
    ...
    if finding_class not in CLASSES:
        raise TicketError(
            f"{finding_id} has no class. Set one with: "
            f"ticket class {ticket['key']} {finding_id} easy|hard"
        )

    if finding_class == "easy":
...
def set_class(store: Store, pr_ref: str, finding_id: str, value: str) -> None:
    if value not in CLASSES:
        raise TicketError(f"class must be one of {', '.join(CLASSES)}")
    doc = store.read_findings(pr_ref)
    _find(doc, finding_id)["class"] = value
    store.write_findings(doc)
```
...
git commit -m "feat: route findings by class and resolve them from commit trailers" -m "Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>"
```
...
**Files:**
- Modify: `src/ticket/cli.py` — add `review`, `collect`, `fix`, `decide`, `class`, `findings`, `reviews`, `open`, `refresh`, `reset`, and the `--pr` / `--force` flags; teach `_execute` the `review`, `collect`, and `fix` actions
...
def test_class_overrides(env, fake_bin, capsys):
    started(fake_bin)
    main(["review", "ABC-123"])
    fake_bin.respond(
        "gh api repos/acme/api/pulls/115/reviews",
    ...
    fake_bin.respond("claude", stdout=json.dumps(["easy", "hard", "easy"]))
    main(["collect", "ABC-123"])
    assert main(["class", "ABC-123", "f02", "easy"]) == 0
    capsys.readouterr()
    main(["findings", "ABC-123", "--json"])
    findings = json.loads(capsys.readouterr().out)
    assert [f["class"] for f in findings if f["id"] == "f02"] == ["easy"]
...
def cmd_class(args) -> int:
    ctx = Context.load()
    ticket = load_ticket(ctx, args.key)
    fix_module.set_class(ctx.store, pick_pr(ticket, args), args.finding, args.value)
...
def cmd_findings(args) -> int:
    ...
    for finding in sorted(
        ...
        klass = finding.get("class") or "unclassified"
        print(
            f"{finding['id']}  {finding.get('status', '?'):<9} "
            f"{finding.get('severity', '?'):<13} {klass:<12} {finding.get('summary', '')}"
        )
    return 0
...
```python
    ...
    p = add("fix", cmd_fix, help="work findings, one commit each, routed by class")
    p.add_argument("key")
    ...
    p = add("class", cmd_class, help="override a finding's class")
    p.add_argument("key")
    p.add_argument("finding")
    p.add_argument("value", choices=list(CLASSES))
    p.add_argument("--pr")
...
Reviews emit the format `parse.py` reads: one `<details>` block per severity keyed by emoji, top-level `*` bullets as findings, a `**Verdict:**` line at the end. **Severity is importance, not effort** — the prompts say so explicitly, and they do not ask for an easy/hard judgement, because that is `classify.py`'s job.
...
ticket fix ABC-123                # work the next finding, routed by class
ticket class ABC-123 f02 hard     # override how a finding gets fixed
...
**Severity and class are different questions.** Severity (🔴 blocking,
🟡 maintenance, 🔵 architecture) says how important a finding is. Class
(`easy`/`hard`) says who can fix it: `easy` goes back to the gh Claude bot as an
`/edit` comment, `hard` gets a local Claude session. A blocking finding can be a
one-line fix. Class is set by Haiku at ingestion and overridden with
`ticket class`; it is never derived from severity.
